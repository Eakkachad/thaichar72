...
def main() -> None:
    ...
    for idx_path, cache_path in zip(args.inputs, caches):
        ...
        df["source_set"] = tag
        frames.append(df)
        dropped = len(new_paths) - kept
        print(f"{idx_path}: {kept}/{len(new_paths)} index rows taken  (tag={tag})")
        if dropped:
            print(f"  WARNING: {dropped} rows have no image in {cache_path} and were dropped. "
                  f"Pass --caches if this index does not pair with its sibling cache.")
