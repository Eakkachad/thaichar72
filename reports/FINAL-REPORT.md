...
— ตารางนี้เป็นสถานะ ณ 2026-09-19 **รายการที่ส่งมอบจริงอยู่ที่ §13.2** ซึ่งเปลี่ยนค่าเริ่มต้นเป็น
`thaichar72_r18_64_gen.pt` และเพิ่ม `_gen_rotaug.pt` (§15.3) ไฟล์ทั้งหมดด้านล่างยังอยู่ครบ ไม่ได้ถูกลบ:

| ไฟล์ | ใช้เมื่อ | ขนาด | md5 (8) |
|---|---|---:|---|
| `thaichar72_resnet18_64.pt` | ค่าเริ่มต้น **ของรอบนี้ (09-19)** — ถูกแทนที่แล้วโดย `thaichar72_r18_64_gen.pt` ใน §13 | 44.9 MB | 467bb628 |
...
## 13. อัปเดต 2026-09-22 — generalisation study และโมเดลที่ส่งมอบใหม่

รายละเอียดทั้งหมดอยู่ใน `reports/02-EXPERIMENTS.md` §K, ที่มาของข้อมูลอยู่ใน `reports/03-DATA-AUDIT-2026-09-22.md`

### 13.1 ทำไมต้องเปลี่ยน

ข้อมูลใหม่ `dataUpdate` (generator + 167 ฟอนต์ + 98,071 ภาพ) ครอบคลุม 35 คลาสที่มีภาพจริงแค่ 1–155 ภาพ
พอเอามาสร้าง **probe ที่กันลายมือและตระกูลฟอนต์ออกทั้งก้อน** แล้ววัดโมเดลเดิม ได้ผลที่ validation split เดิมมองไม่เห็นเลย:

| | strat val | probe (35 คลาสหาง, ลายมือไม่เคยเห็น) |
|---|---:|---:|
| recipe เดิม (F19 line) | 0.9902 | **0.2258** |
| recipe ใหม่ (G4) | 0.9902 | **0.4292** |

**ครึ่งหนึ่งของ class list แทบไม่ได้ถูกทดสอบมาตลอด** — ฃ และ ฑ มีภาพจริงคลาสละ 1 ภาพและ **ไม่มีภาพใน validation เลย**
recall บน probe = 0.000 ถ้า hidden test เข้าใกล้ class-balanced ตรงนี้คือจุดที่เสียคะแนน

### 13.2 โมเดลที่ส่งมอบ (เพิ่ม ไม่ได้ลบของเดิม)

| ไฟล์ | คืออะไร | ขนาด | md5 |
|---|---|---:|---|
| `thaichar72_r18_64_gen.pt` | **ค่าเริ่มต้นใหม่** — G4 (stage-1 203 ฟอนต์ + dataUpdate → fine-tune ข้อมูลจริง) | 44.9 MB | 90ea7a3a |
| `thaichar72_r18_64_gen_fp16.pt` | เหมือนกันแบบ fp16 (ผลเท่ากันทุกหลัก) | 22.5 MB | 96f9c8d4 |
| `thaichar72_r18_synth203du_pretrain_init.pt` | stage-1 init ที่ทำให้ได้ผลข้างบน — ทำซ้ำได้โดยไม่ต้อง render ใหม่ | 44.9 MB | 30aa5ef0 |
| `thaichar72_resnet18_64.pt` | ของเดิม (K1) **ยังอยู่ครบ ไม่ถูกแตะ** | 44.9 MB | 467bb628 |

`configs/final.yaml` ชี้ไปที่ recipe ใหม่แล้ว ของเดิมเก็บไว้ที่ `configs/final_f19_v2.yaml` (คัดลอกทับกลับได้ทันที)
`configs/final.yaml` เดิมมีปัญหาอยู่ก่อนแล้วบนเครื่องใหม่: `init_from` ชี้ไป `runs/*/best.pt` ซึ่งถูก gitignore
ตัวใหม่ชี้ไป `weights/` ที่ packaged มาด้วย จึงใช้ได้จาก fresh clone

### 13.3 เกณฑ์ที่ใช้เลือก (doc-disjoint + robustness gate ไม่ใช่ stratified val)

| | F19_ctrl (baseline บน split เดียวกัน) | **G4 (ส่งมอบ)** |
|---|---:|---:|
| doc-disjoint top-1 | 0.9856 | **0.9872** |
| robustness gate (เกณฑ์ ≥ 0.8988) | 0.9124 ✅ | **0.9153** ✅ |
| tail probe (ลายมือไม่เคยเห็น) | 0.2258 | **0.4292** |
| stratified val top-1 | 0.9902 | 0.9902 |

3 seeds ของ G4 = 0.9899 ± 0.0004 → ระยะห่าง +0.16 จุดบน doc **ใหญ่กว่า noise ระหว่าง seed** ส่วนบน strat เป็น 0.00 จุด

### 13.4 สิ่งที่ลองแล้วไม่ได้ผล (บันทึกไว้เพื่อไม่ให้ทำซ้ำ)

- **font diversity อย่างเดียวไม่พอ** — G3 ใช้ 203 ฟอนต์แต่ไม่มี dataUpdate ได้ผลเท่า baseline ที่ใช้ 26 ฟอนต์
  ทั้ง gate และ probe ตัวที่สร้างความต่างคือ **ข้อมูลของคลาสหาง** ไม่ใช่จำนวนฟอนต์
- **เติม synthetic ตอน fine-tune (G5) ทำให้แย่ลง** — ตกที่ gate ต่ำสุด (0.9041) และ macro-F1 บน doc ต่ำกว่าชาวบ้าน
  (0.9703 vs 0.9798) ยืนยันผลเดิมของโปรเจกต์ว่า synthetic เหมาะเป็น pretraining มากกว่าเป็น extra data
- **KD รอบนี้ไม่ช่วย** — ต่างจาก K1: K2 แพ้ G4 ตัวเดียวทั้ง doc และ probe เพราะ 3 seeds ของ G4 เหมือนกันเกินไป
  (±0.0004) ensemble ได้แค่ +0.06 จุด ไม่พอให้กลั่น

### 13.5 ข้อจำกัดที่ต้องอ่านก่อนใช้ตัวเลข probe

probe เป็น **ข้อมูลสังเคราะห์** จาก generator ตัวเดียว ใช้**เทียบอันดับระหว่างโมเดลที่ fine-tune เหมือนกัน**ได้
แต่**อ่านเป็นคะแนนที่จะได้บน hidden test ไม่ได้** ทดสอบหาสาเหตุของตัวเลขที่ต่ำแล้วไม่มีตัวแปรไหนมีนัยสำคัญ
(จำนวนภาพจริง p=0.086, aspect-ratio gap p=0.28, ink gap p=0.80 — 34 คลาสน้อยเกินไป) และ **G5 เทียบด้วย probe ไม่ได้เลย**
เพราะเป็นตัวเดียวที่เห็น domain ของ dataUpdate ตอน fine-tune

---

## 14. อัปเดต 2026-09-22 (ต่อ) — เตรียมส่งงานแบบ hackathon และการทำความสะอาด label

รายละเอียดอยู่ใน `reports/02-EXPERIMENTS.md` §M

### 14.1 รูปแบบการส่งงานเปลี่ยนวิธีเตรียม

วันส่งงานเป็นแบบ hackathon: ไม่มีข้อมูลบอกล่วงหน้า เอาโมเดลไปรันบนข้อมูลจริงหน้างาน ดูผลซ้ำได้ภายใน
1 ชั่วโมง แล้วเลือกอันที่ดีที่สุด สิ่งที่ตัดสินผลจึงไม่ใช่แค่โมเดล แต่คือ **ความเร็วในการลองทุกตัว** และ
**ความทนทานต่อรูปแบบ input ที่เราไม่ได้ควบคุม**

### 14.2 จุดเปราะที่พบและแก้แล้ว — accuracy แกว่ง 0.99 ถึง 0.49 ตามการจัดกรอบภาพ

`preprocess_image` ตัดสินขั้วสี (หมึกเข้มบนพื้นขาว หรือกลับกัน) จากวงขอบ 1 พิกเซล ซึ่งถูกเมื่อมีกระดาษ
ล้อมตัวอักษร แต่ผิดเมื่อภาพถูก crop ชิดจนเส้นแตะขอบ หรือถูก pad ด้วยศูนย์ (กรอบดำ) วัดบน 493 ภาพ
held-out ด้วยโมเดลที่ส่งมอบ:

| รูปแบบ input | ก่อนแก้ | หลังแก้ |
|---|---:|---:|
| framing สมบูรณ์ | 0.9939 | 0.9696 |
| crop ชิดไม่มีขอบ | 0.9229 | 0.9696 |
| crop ชิด + กลับขั้วสี | 0.9026 | 0.9736 |
| **pad ด้วยศูนย์ (กรอบดำ)** | **0.4949** | **0.9696** |
| กลับขั้วสี + กรอบขาว | 0.4949 | 0.9736 |
| สแกนละเอียดกว่า 4 เท่า | 0.9148 | 0.9513 |

`_reframe()` ลอกวงขอบที่มีค่าคงที่ออกก่อน (กรอบที่คนอื่นเติมมาไม่ใช่หลักฐานว่ากระดาษสีอะไร) แล้วเติมขอบใหม่
ด้วยสีพื้นหลังที่มุมภาพบอก **ทุกรูปแบบให้ผลเท่ากันหมดแทนที่จะแกว่งตามดวง** แลกกับ −2.4 จุดบน input ที่
framing สมบูรณ์ ซึ่งคุ้มเมื่อไม่รู้รูปแบบล่วงหน้า และ crop ของ round2 เองก็ชิด

### 14.3 เครื่องมือหน้างาน

```bash
uv run --no-sync python scripts/evaluate_folder.py --dir /path/to/test_data
```

ชี้ที่โฟลเดอร์เดียว → หาภาพ recursive, อ่าน label จากชื่อโฟลเดอร์ (`161`, `161ก`, หรือตัวอักษรไทย) หรือ
`--labels-csv`, `--no-labels` ถ้าไม่มีเฉลย, **รันทุก weight แล้วจัดอันดับ** (493 ภาพ × 5 โมเดล ≈ 4 วินาที),
บอกว่าชุดนี้มีกี่คลาส สมดุลไหม ภาพไหนอ่าน label ไม่ออก, ลองทั้งสองขั้วสีแล้วเลือกอันที่มั่นใจกว่า
และเตือนถ้าทั้งชุดดูกลับขั้ว

### 14.4 โมเดลที่พกไป 5 ตัว — ให้ข้อมูลจริงตัดสิน ไม่ต้องเดา

| ไฟล์ | คือ | เผื่อกรณี |
|---|---|---|
| `thaichar72_r18_64_gen.pt` | G4, label v2 | ค่าเริ่มต้น |
| `thaichar72_r18_64_gen_v3labels.pt` | recipe เดียวกัน, label v3 | เฉลยสะอาดกว่าของเรา |
| `thaichar72_resnet18_64.pt` | K1 เดิม | — |
| `thaichar72_resnet18_64_v1labels.pt` | ก่อนแก้ DataV2 | เฉลยใช้ convention เดิม |
| `thaichar72_mnv3small_64_small.pt` | 1.6 M params | ถ้าต้องเร็ว/เล็ก |

เราไม่รู้ว่าเฉลยของอาจารย์ใช้ convention ไหน และ**ไม่จำเป็นต้องรู้** เพราะรูปแบบ hackathon ให้ลองได้

### 14.5 การทำความสะอาด label — ทำแล้ว แต่ไม่ได้คะแนน

หา 444 ภาพต้องสงสัยด้วย out-of-fold (ทุกภาพถูกทำนายโดยโมเดลที่ไม่เคยเห็นมัน) เจ้าของงานยืนยันด้วยตา
**146 ภาพใน 10 คู่** ว่าเฉลยผิดจริง แก้แล้วเป็น `split_seed42_v3.csv`

วัดแบบตัดภาพที่แก้ออกและให้ทั้งสองโมเดลใช้เฉลยชุดเดียวกัน 3 seeds ต่อฝั่ง:
**doc top-1 −0.10 จุด (p=0.65), doc balanced +0.27 จุด (p=0.24)** → **ไม่ต่างกัน** เพราะแก้ไป 120 ภาพ
จากชุดเทรน 47,949 ภาพ = 0.25 %

ข้อสรุปที่ใช้ตัดสินใจต่อได้: **label noise ไม่ใช่คอขวด** คอขวดคือ**การวัด** — 8 คลาสไม่มีภาพ validation เลย
ถ้าจะลงแรงเพิ่ม ให้ไป label ภาพจริงของคลาสที่ขาด ไม่ใช่ไล่แก้ label ที่มีอยู่

---

## 15. อัปเดต 2026-09-23 — ความทนทานต่อ input ที่ไม่เคยเทรน และชุดเครื่องมือวันส่งงาน

รายละเอียดเต็มอยู่ใน `reports/02-EXPERIMENTS.md` §N และ `reports/stress/summary.md`

### 15.1 บั๊กที่ต้องบอกก่อน — ตัวเลข inference ที่รายงานไว้ก่อนหน้านี้ต่ำกว่าความจริง 2 จุด

ตอนสร้างชุดทดสอบความทนทาน เกณฑ์ตรวจสอบบังคับว่าค่า clean ต้องตรงกับ val ที่รู้ ปรากฏว่าไม่ตรง
พอเทียบสองเส้นทางบนภาพชุดเดียวกัน 492 ภาพ: เส้นทางตอน validate ได้ **0.9898** แต่เส้นทาง inference
ที่ใช้จริง (`predict.py`, `evaluate_folder.py`, notebook) ได้ **0.9695** — ผิด 10 ภาพ ถูก 0

สาเหตุ: `_reframe` เติมขอบภาพด้วย **median ของสี 4 มุม** ซึ่ง glyph ที่ crop ชิดจนเส้นแตะมุมจะได้สีหมึก
Otsu จึงอ่านว่าขอบคือพื้นหลังแล้วกลับขั้วทั้งภาพ และเพราะการเดาผิดอยู่ที่*ค่าที่เอาไปเติม* ไม่ใช่ที่ขั้วของ input
ตัวเลือก `--polarity both` ที่เขียนไว้กันเรื่องนี้จึง**ไม่เคยทำงาน** แก้เป็นใช้ค่าที่สว่างที่สุดของมุม ได้ 0.9898 ตรงเป๊ะ
**ตัวเลขฝั่ง inference ทั้งหมดก่อนการแก้นี้ต่ำกว่าความจริงประมาณ 2 จุด** ส่วนตัวเลขบนตารางผลการทดลองไม่กระทบ
เพราะใช้คนละเส้นทาง

### 15.2 โมเดลทนอะไรได้ ไม่ทนอะไร

| input | ผล | อ่านว่า |
|---|---|---|
| ภาพจริงเล็กกว่า 9 px | **0.974** | ความละเอียดต่ำไม่ใช่ปัญหา ชุดข้อมูลมีแบบนี้ 8.5 % อยู่แล้ว |
| JPEG คุณภาพ 5 | ~1.00 | Otsu ล้าง artifact ทิ้งหมด |
| ความละเอียดสูง 8× | ~1.00 | สแกนละเอียดกว่าเดิมไม่กระทบ |
| สัดส่วนเพี้ยน 0.7–1.4 | ~0.95–1.00 | resize ไม่รักษาสัดส่วนเสียเล็กน้อย |
| ฟอนต์ไม่เคยเห็น 62 แบบ | **0.868** | ชนะรุ่นก่อน (K1) 3.4 จุด — ผลของการ pretrain 203 ฟอนต์ |
| **หมุน 30°** | 0.52 → **0.90** | แก้ได้ด้วย `--rotation-tta` |
| **กัดเส้น 4 px** | 0.25 | จุดอ่อนที่แก้ไม่ได้ — **ห้ามมีขั้นที่ทำให้เส้นบางลง** |
| ตัวอักษรติดกัน/ครึ่งตัว/ขีดมั่ว | AUROC ≥ 0.84 | กรองด้วย threshold ความมั่นใจได้ ไม่ต้องสร้างตัวแยก |

### 15.3 การหมุน — จุดอ่อนใหญ่ที่สุดที่แก้ได้ และแก้แล้ว

กระดาษที่ถ่ายรูปหรือวางเอียง 10–30° เป็นเรื่องปกติ แต่ augmentation เดิมหมุนแค่ 8° และการทดสอบเดิมหยุดที่ 20°

`predict_with_rotation_search` หมุนภาพไปทีละมุม (±10/20/30/45) แล้วเลือกคำตอบที่โมเดลมั่นใจที่สุด
โดยค้นเฉพาะภาพที่อ่านตั้งตรงแล้วไม่มั่นใจ (เข้าเงื่อนไขแค่ 3 % ของภาพปกติ) และยอมเปลี่ยนคำตอบก็ต่อเมื่อ
มุมใหม่ชนะเกิน 0.30 **ผล: +37.7 จุดที่ 30°, +75.1 จุดที่ 45°, ไม่เสีย clean เลย**
ทดสอบกับไฟล์จริงผ่านเครื่องมือวันส่งงาน: โฟลเดอร์ที่หมุน 35° ได้ 0.2429 → **0.8286**

### 15.4 ขั้นตอนวันส่งงาน

```bash
uv run python scripts/triage_input.py   --dir <โฟลเดอร์ข้อมูลทดสอบ>
uv run python scripts/evaluate_folder.py --dir <โฟลเดอร์> [--rotation-tta]
```

`triage_input.py` อ่านภาพโดยไม่ต้องมี label แล้วบอกว่าข้อมูลชุดนี้ความละเอียดเท่าไหร่ เส้นหนาบาง
ขั้วสีกลับไหม หมุนไหม แล้วพิมพ์คำสั่งที่ควรใช้ออกมาให้ การตรวจการหมุนใช้ **ความสอดคล้องของมุมที่ชนะ**
ไม่ใช่การที่ความมั่นใจสูงขึ้น เพราะอย่างหลังเกิดกับภาพเสียทุกแบบ ไม่ใช่เฉพาะภาพหมุน

### 15.5 สิ่งที่ยังวัดไม่ได้

**8 คลาส (ฃ ฑ ฤ ฬ ฯ ฮ ๔ ๕) ไม่มีภาพ validation จริงเลยสักภาพ** และ 24 คลาสที่มีข้อมูลน้อยกว่า 50 ภาพ
รวมกันมีภาพใน doc-val แค่ 123 ภาพ ผลเรื่องคลาสหางทั้งหมดใน §13–§14 ถูกจำกัดด้วยข้อนี้ ไม่ใช่ด้วยตัวโมเดล
**ทางแก้คือหาภาพจริงของ 35 ตัวอักษรนี้มาเป็นชุดทดสอบ** ซึ่งใช้เวลาคนไม่กี่ชั่วโมง และควรถามอาจารย์ว่า
hidden test กระจายแบบสมดุลไหม เพราะคำตอบเปลี่ยนว่างานคลาสหางคุ้มแค่ไหนตั้งแต่ต้น
